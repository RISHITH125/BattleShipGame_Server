...
const handleMessage = (bytes, uuid) => {
  ...
  if (message.action === "create room") {
    ...
    connections[uuid].send(
      JSON.stringify({ action: "Waiting for the player2", RoomId: user.roomId })
    );
    ...
    if (room && room.clients.length < 2) {
      ...
      const otherPlayerUuid = room.clients.find(
        (clientUuid) => clientUuid != uuid
      );
      const otherPlayer = users[otherPlayerUuid];

      connections[uuid].send(JSON.stringify({ action: "JoinedRoom" }));
      connections[otherPlayerUuid].send(
        JSON.stringify({ action: "JoinedRoom" })
      );

      setTimeout(() => {
        connections[uuid].send(
          JSON.stringify({
            action: "BroadCastName",
        ...
        connections[otherPlayerUuid].send(
          JSON.stringify({ action: "SelectShips" })
        ); // 1st player
      }, 50);
    } else {
  ...
  if (message.action === "ShipsSelectionComplete") {
    ...
    if (Array.isArray(selectedShips)) {
      ...
      if (
        ...
        room.clients.every(
          (clientUuid) =>
            Array.isArray(users[clientUuid].MyShips) &&
            users[clientUuid].MyShips.length > 0
        )
      ) {
        // If they have, send a message to the room creator that it's their turn
        const roomCreatorUuid = room.owner;
        const player2uuid = room.clients.find(
          (clientUuid) => clientUuid !== uuid
        );
        const player2Connection = connections[player2uuid];
        if (!users[roomCreatorUuid].Myturn) {
          // Check if the turn has already been set
          users[roomCreatorUuid].Myturn = true;
          setTimeout(() => {
            player2Connection.send(
              JSON.stringify({ action: "Let the game begin" })
            );
          }, 100);
          connections[roomCreatorUuid].send(
            JSON.stringify({
              action: "My turn",
              turn: users[roomCreatorUuid].Myturn,
            })
          );
          // player2Connection.send(JSON.stringify({ action: "Let the game begin"}));

          if (player2uuid !== roomCreatorUuid) {
            // Check if player2 is not the owner
            setTimeout(() => {
              player2Connection.send(
                JSON.stringify({ action: "Let the game begin" })
              );
      ...
      console.log(
        `Invalid data received for setting ships for user ${users[uuid].username}`
      );
    }
  }
  if (message.action === "turn complete") {
    ...
    if (indexExists) {
      ...
      const player2Connection = connections[player2uuid];
      player2Connection.send(
        JSON.stringify({ action: "health update", health: player2.MyHealth })
      );
      connections[uuid].send(
        JSON.stringify({ action: "index check", exists: true })
      );
    } else {
      connections[uuid].send(
        JSON.stringify({ action: "index check", exists: false })
      );
    }
    ...
    if (user.turns >= 7) {
      ...
      connections[uuid].send(
        JSON.stringify({ action: "Game over", whowin: winner })
      );
      connections[player2uuid].send(
        JSON.stringify({ action: "Game over", whowin: winner })
      );
    } else {
      ...
      connections[uuid].send(JSON.stringify({ action: "Opponent turn" }));
      player2Connection.send(
        JSON.stringify({ action: "My turn", turn: player2.Myturn })
      );
    }
  }
};
  ...
  if (room) {
    ...
    if (room.clients.length === 0) {
      ...
      connections[otherPlayerUuid].send(
        JSON.stringify({
          action: "Game over",
          whowin: users[otherPlayerUuid].username,
        })
      );
    }
  }
